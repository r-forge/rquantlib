...
\begin{abstract}
  ...
  \pkg{RQuantLib} is still the only \proglang{R} package that brings the
  quantitative analytics of QuantLib to \proglang{R} while conencting the
  rich interactive \proglang{R} environment for data analysis, statistics and
  visualization for direct use with QuantLib. Besides providing an convenient
  and easy access to QuantLib for \proglang{R} users who do not have the
  necessary experience in C++ to employ QuantLib directly, it also sets up a
  framework for users who wants to interface their own QuantLib-based
  functions with \proglang{R}.
